...
public class ArgProcessor {
    ...
    public ProcessingResult process() {
        if (args.length < 1) 
        {
            return new ProcessingResult(Consts.ERR_FILE_NAME_MISSING, PRINT_USAGE);
        }
        ...
        for (int i = 0; i < args.length; i++) 
        {
            if (Objects.equals(args[i], "d")) 
            {
                if (i + 1 < args.length) 
                {
                    ...
                    return new ProcessingResult(Consts.ERR_MISSING_DELIMITERS, PRINT_USAGE);
                }
            }
            else if (Objects.equals(args[i], "-l")) 
            {
                if (i + 1 < args.length) 
                {
                    try 
                    {
                    	...
                    	if (minWordLength <= 0)
                    	{
                            return new ProcessingResult(Consts.ERR_MIN_LENGTH_SHOULD_BE_GREATER_THAN_0 + args[i + 1], PRINT_USAGE);
                    	}
                    	avgSentenceLength.setMinWordLength(minWordLength);
                    }
                    catch (NumberFormatException e) 
                    {
                        return new ProcessingResult(Consts.ERR_INVALID_MIN_LENGTH + ": " + args[i + 1], PRINT_USAGE);
                    }
                    i++;
                } 
                else 
                {
                    return new ProcessingResult(Consts.ERR_MISSING_MIN_LENGTH, PRINT_USAGE);
                }
            }
            else
            {
        ...
        if (fileName != null) 
        {
            ...
            {
                return new ProcessingResult(Consts.ERR_FILE_DOES_NOT_EXIST + ": " + fileName, SYSTEM_EXIT);
            }
        } 
        else 
        {
